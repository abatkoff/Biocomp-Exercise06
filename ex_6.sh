...
cat wages.csv | cut -d, -f 2 > justwages.txt

for variable in justwages.csv
do 
echo "(16yrs=$(cat justwages.txt | grep ",16," justwages.csv | sort -n | head -n 1 | cut -d, -f 3)) - (12yrs= $(cat justwages.txt | grep ",12," justwages.csv | sort -n | head -n 1 | cut -d, -f 3))"
done
